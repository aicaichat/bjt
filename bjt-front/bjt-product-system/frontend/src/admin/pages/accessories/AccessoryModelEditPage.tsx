...
const AccessoryModelEditPage: React.FC = () => {
  ...
  const isEditMode = !!id;
  const title = isEditMode ? '编辑配件型号' : '新增配件型号';
  ...
  useEffect(() => {
    ...
    const fetchAccessoryModel = async () => {
      setLoading(true);
      try {
        const model = await accessoryModelService.getAccessoryModel(Number(id));
  ...
  const handleSubmit = async (values: AccessoryModelFormData) => {
    ...
    try {
      if (isEditMode) {
        await accessoryModelService.updateAccessoryModel(Number(id), values);
      ...
      message.error(isEditMode ? '更新配件型号失败' : '创建配件型号失败');
    } finally {
      setSubmitting(false);
    }
  };
