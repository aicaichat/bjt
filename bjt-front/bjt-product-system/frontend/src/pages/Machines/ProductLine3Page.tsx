...
const ProductLine3Page: React.FC = () => {
  ...
  const getMachineCacheKey = (category: string, region: string, voltage: string, frequency: string) => `machines_${category}_${region}_${voltage}_${frequency}`;
  ...
  const fetchMachines = async () => {
    ...
    try {
      // 修改：缓存键包含频率
      const cacheKey = getMachineCacheKey(category, filterRegion, selectedVoltage, selectedFrequency);
      ...
      if (cachedData) {
        try {
          ...
          if (parsedData.timestamp && Date.now() - parsedData.timestamp < 5 * 60 * 1000) {
            setMachines(parsedData.data);
            setTotal(parsedData.total);
            setTotalPages(Math.ceil(parsedData.total / pageSize));
            setLoading(false);
            return;
          }
        } catch (e) {
            ...
            if (machinesData && machinesData.length > 0) {
              ...
              try {
                localStorage.setItem(
                  getMachineCacheKey(category, filterRegion, selectedVoltage, selectedFrequency),
                  JSON.stringify({ timestamp: Date.now(), data: transformedMachines, total: data.data.total })
                );
              } catch (cacheSaveErr) {
  ...
  useEffect(() => {
    fetchMachines();
    fetchHostModels();
  }, [category, currentLanguage, filterRegion, selectedVoltage, selectedFrequency]);
  ...
  const filteredMachines = React.useMemo(() => {
    ...
    return machines.filter(machine => {
      ...
      // 频率筛选
      const frequencyMatch = selectedFrequency === 'ALL' || !selectedFrequency || machine.frequency === selectedFrequency;
      return modelMatch && voltageMatch && frequencyMatch;
    });
  }, [machines, filterType, selectedVoltage, selectedFrequency]);
  ...
  const frequencyOptions = React.useMemo(() => {
    if (!Array.isArray(machines)) return [];
    const uniqueFrequencies = Array.from(new Set(machines.map(m => m.frequency).filter(Boolean)));
    
    return [
      { value: 'ALL', label: t('filters.allFrequencies') || '全部频率' },
  ...
  return (
    <div className="machines-page min-h-screen bg-gray-50 text-gray-900">
      ...
      <div className="bg-white rounded-lg shadow-md p-4 mb-6 text-gray-900 border border-gray-200 transition-colors duration-300">
        <h1 className="text-xl font-bold mb-4 text-gray-800">{t('pageTitle')} - {t('productLines.tapeSystem')}</h1>
...
export default ProductLine3Page;