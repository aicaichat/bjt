...
|--------|------|------|------|
| id | bigint(20) | 主键ID | PRIMARY KEY, AUTO_INCREMENT |
| product_line_id | bigint(20) | 产品线ID | NOT NULL, FOREIGN KEY |
| product_id | varchar(100) | 产品ID | NOT NULL, UNIQUE |
| model | varchar(100) | 主机型号编码 | NOT NULL, UNIQUE |
| voltage_options | varchar(255) | 电压选项，JSON格式 | |
...
注：app_model 字段用于存储耗材适用的设备型号，多个型号之间使用逗号分隔，例如："LP-V1,LP-V2,LP-V3"。通过这个字段可以直接判断耗材与设备的兼容性，无需额外的关联表。

### 2.12 耗材规格表 (consumable_specifications)
存储耗材规格的标准化信息。

| 字段名 | 类型 | 说明 | 约束 |
|--------|------|------|------|
| id | bigint(20) | 主键ID | PRIMARY KEY, AUTO_INCREMENT |
| consumable_id | bigint(20) | 耗材ID | NOT NULL, FOREIGN KEY |
| spec_key | varchar(50) | 规格键名 | NOT NULL |
| spec_value_metric | varchar(100) | 规格值(公制) | |
| spec_value_imperial | varchar(100) | 规格值(英制) | |
| display_order | int(11) | 显示顺序 | DEFAULT 0 |
| created_at | datetime | 创建时间 | DEFAULT CURRENT_TIMESTAMP |
| updated_at | datetime | 更新时间 | DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP |
...
### 2.9 必选备件表 (required_accessories)
存储备件之间的必选关系。

| 字段名 | 类型 | 说明 | 约束 |
|--------|------|------|------|
| id | bigint(20) | 主键ID | PRIMARY KEY, AUTO_INCREMENT |
| accessory_id | bigint(20) | 配件ID | NOT NULL, FOREIGN KEY |
| required_accessory_id | bigint(20) | 必选备件ID | NOT NULL, FOREIGN KEY |
| quantity | int(11) | 必选数量 | NOT NULL, DEFAULT 1 |
| description | varchar(255) | 必选说明 | |
| created_at | datetime | 创建时间 | DEFAULT CURRENT_TIMESTAMP |
| updated_at | datetime | 更新时间 | DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP |
...
- FOREIGN KEY (required_accessory_id) REFERENCES accessories(id)

### 2.10 关联关系表 (relations)
存储产品线、型号、料号之间的多对多关系。
...
8. 产品线表 (lines) 与耗材表 (consumables) 建立一对多关系
   - 通过 product_line_id 字段关联
   - 一个产品线可以包含多个耗材
   - 一个耗材必须属于一个产品线

9. 耗材表 (consumables) 与耗材规格表 (consumable_specifications) 建立一对多关系
   - 通过 consumable_id 字段关联
   - 一个耗材可以有多个规格
   - 规格必须属于一个耗材
...
8. 耗材规格表索引：
   ...
   - UNIQUE KEY unique_spec (consumable_id, spec_key)
9. 耗材兼容性表索引：
   - PRIMARY KEY (id)
   - KEY consumable_id (consumable_id)
   - KEY target_type_id (target_type, target_id)
   - UNIQUE KEY unique_compatibility (consumable_id, target_type, target_id)
10. 耗材价格区间表索引：
    - PRIMARY KEY (id)
    - KEY consumable_id (consumable_id)
    - UNIQUE KEY unique_quantity_range (consumable_id, min_quantity, max_quantity)
