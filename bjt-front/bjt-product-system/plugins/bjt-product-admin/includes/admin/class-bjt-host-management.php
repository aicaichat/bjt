...
class BJT_Host_Management {
    ...
    private function __construct() {
        global $wpdb;
        $this->tables = array(
            'hosts' => $wpdb->prefix . 'bjt_hosts',
            'part_numbers' => $wpdb->prefix . 'bjt_part_numbers'
        );
     ...
     * Get machines list
     */
    public function get_machines($request) {
        ...
        global $wpdb;
        $table_name = $wpdb->prefix . 'bjt_hosts';
        ...
        $items = $wpdb->get_results($wpdb->prepare(
            "SELECT 
                ...
                h.description_$lang as description,
                h.specifications_$lang as specifications,
                h.image_url,
            ...
            FROM $table_name h
            LEFT JOIN {$wpdb->prefix}bjt_prices p ON h.model = p.part_number AND p.region = %s
            LEFT JOIN {$wpdb->prefix}bjt_inventory i ON h.model = i.part_number AND i.warehouse = 'MAIN'
     ...
     * Get machine detail
     */
    public function get_machine($request) {
        ...
        // 处理规格
        $machine->specifications = json_decode($machine->specifications);
        ...
        $machine->price = array(
            'base' => (float)$machine->base_price,
            'currency' => $machine->currency,
            'tiers' => $price_tiers
        );

        // 处理库存
        $machine->inventory = array(
            'available' => (int)$machine->available,
            'reserved' => (int)$machine->reserved,
            'next_arrival' => $machine->next_arrival
        );
        ...
        $accessories = $wpdb->get_results($wpdb->prepare(
            ...
            JOIN {$wpdb->prefix}bjt_accessory_relations ar ON a.model = ar.accessory_model
            WHERE ar.host_model = %s
        ...
        // 清理不需要的字段
        unset($machine->base_price, $machine->currency, $machine->available, $machine->reserved, $machine->next_arrival);
