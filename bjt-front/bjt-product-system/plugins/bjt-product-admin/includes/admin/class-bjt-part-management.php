...
class BJT_Part_Management {
    ...
    public function get_consumables($request) {
        ...
        // 构建查询条件
        $where = array('status = "publish" AND type = "consumable"');
        $params = array();
        ...
        if ($machine_id) {
            $where[] = 'id IN (SELECT part_id FROM ' . $wpdb->prefix . 'bjt_machine_parts WHERE machine_id = %d)';
            $params[] = $machine_id;
        }
        ...
        $items = $wpdb->get_results($wpdb->prepare(
            "SELECT 
                ...
                p.specifications_$lang as specifications,
                p.image_url,
            ...
            FROM $table_name p
            LEFT JOIN {$wpdb->prefix}bjt_prices pr ON p.model = pr.part_number AND pr.region = %s
            LEFT JOIN {$wpdb->prefix}bjt_inventory i ON p.model = i.part_number AND i.warehouse = 'MAIN'
    ...
    public function get_consumable($request) {
        ...
        $consumable = $wpdb->get_row($wpdb->prepare(
            ...
            LEFT JOIN {$wpdb->prefix}bjt_prices pr ON p.model = pr.part_number AND pr.region = %s
            LEFT JOIN {$wpdb->prefix}bjt_inventory i ON p.model = i.part_number AND i.warehouse = 'MAIN'
            WHERE p.id = %d AND p.status = 'publish' AND p.type = 'consumable'",
            $region,
            $id
        ));
        ...
        // 处理规格
        $consumable->specifications = json_decode($consumable->specifications);
        ...
        $consumable->price = array(
            'base' => (float)$consumable->base_price,
            'currency' => $consumable->currency,
            'tiers' => $price_tiers
        );

        // 处理库存
        $consumable->inventory = array(
            'available' => (int)$consumable->available,
            'reserved' => (int)$consumable->reserved,
            'next_arrival' => $consumable->next_arrival
        );
        ...
        $compatible_machines = $wpdb->get_results($wpdb->prepare(
            ...
            JOIN {$wpdb->prefix}bjt_machine_parts mp ON h.id = mp.machine_id
            WHERE mp.part_id = %d AND h.status = 'publish'
            ORDER BY h.menu_order ASC",
            $id
        ));

        $consumable->compatible_machines = $compatible_machines;

        // 清理不需要的字段
        unset($consumable->base_price, $consumable->currency, $consumable->available, $consumable->reserved, $consumable->next_arrival);
