...
<div class="wrap">
    ...
    <div class="notice notice-info">
        <p><?php _e('Manage parts, accessories, and consumables. Changes will be immediately reflected in the product catalog.', 'bjt-product-admin'); ?></p>
    </div>
    ...
    <div class="tablenav top">
        <div class="alignleft actions">
            <form method="get" class="filter-form">
                ...
                <select name="part_type" id="part-type-filter">
                    <option value=""><?php _e('All Types', 'bjt-product-admin'); ?></option>
                    <option value="accessory" <?php selected(isset($_GET['part_type']) && $_GET['part_type'] === 'accessory'); ?>>
                        <?php _e('Accessories', 'bjt-product-admin'); ?>
                    </option>
                    <option value="consumable" <?php selected(isset($_GET['part_type']) && $_GET['part_type'] === 'consumable'); ?>>
                        <?php _e('Consumables', 'bjt-product-admin'); ?>
                    </option>
                </select>

                <input type="text" name="part_number" placeholder="<?php _e('Part Number', 'bjt-product-admin'); ?>" 
                       value="<?php echo isset($_GET['part_number']) ? esc_attr($_GET['part_number']) : ''; ?>">
        ...
        <div class="alignright">
            <button class="button" id="export-parts"><?php _e('Export', 'bjt-product-admin'); ?></button>
            <label for="import-parts" class="button">
    ...
    <table class="wp-list-table widefat fixed striped">
        ...
        <tbody id="the-list">
            <?php
            // Get filtered parts
            $parts = $parts_manager->get_filtered_parts(
                isset($_GET['part_type']) ? $_GET['part_type'] : '',
                isset($_GET['part_number']) ? $_GET['part_number'] : ''
            );

            if (empty($parts)) {
                echo '<tr><td colspan="6">' . __('No parts found.', 'bjt-product-admin') . '</td></tr>';
            } else {
                foreach ($parts as $part) {
                    $host = $parts_manager->get_host($part['host_id']);
                    $status_class = $part['status'] === 'active' ? 'status-active' : 'status-inactive';
                    ?>
                    <tr>
                        ...
                        <td>
                            <?php 
                            if ($host) {
                                echo esc_html($host['title_cn']) . '<br><small>' . esc_html($host['title_en']) . '</small>';
                            } else {
                                _e('None', 'bjt-product-admin');
                            }
                            ?>
                        </td>
                        <td>
                            <span class="status-indicator <?php echo $status_class; ?>"></span>
                            <?php echo $part['status'] === 'active' ? __('Active', 'bjt-product-admin') : __('Inactive', 'bjt-product-admin'); ?>
                        </td>
                        <td class="actions">
                            ...
                            <button type="button" class="button button-small toggle-status" 
                                ...
                                <?php echo $part['status'] === 'active' ? __('Deactivate', 'bjt-product-admin') : __('Activate', 'bjt-product-admin'); ?>
                            </button>
...
jQuery(document).ready(function($) {
    ...
    $('#export-parts').on('click', function() {
        window.location.href = ajaxurl + '?' + $.param({
            ...
            nonce: '<?php echo wp_create_nonce("bjt_product_admin_nonce"); ?>',
            part_type: $('#part-type-filter').val(),
