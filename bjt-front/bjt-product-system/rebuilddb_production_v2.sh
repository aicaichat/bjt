...
# Production Database Rebuild Script V2
# 生产环境数据库重建脚本 - 保留WordPress核心表版本
...
echo "=== BJT Production Database Rebuild Script V2 ==="
echo "WARNING: This will rebuild BJT data but preserve WordPress core tables!"
...
# 确认操作
read -p "Are you sure you want to proceed? This will DELETE BJT data but keep WordPress core! (yes/no): " confirm
if [ "$confirm" != "yes" ]; then
...
docker exec -i $DB_HOST mysqldump -u$DB_USER -p$DB_PASSWORD $DB_NAME > "bjt_backup_$(date +%Y%m%d_%H%M%S).sql"
if [ $? -eq 0 ]; then
...
echo "Dropping existing BJT tables from '$DB_NAME'..."
...
DROP TABLE IF EXISTS wp_bjt_carts;
DROP TABLE IF EXISTS wp_bjt_machines;
...
docker exec -i $DB_HOST mysql -u$DB_USER -p$DB_PASSWORD -e "USE $DB_NAME; SELECT option_name, option_value FROM wp_options WHERE option_name LIKE '%jwt%';" || exit 1
...
echo ""
echo "=== Production Database Rebuild V2 Complete! ==="
echo "Database: $DB_NAME"
echo "BJT data has been rebuilt with proper UTF-8 encoding."
echo "WordPress core tables have been preserved."
echo "Test users have been created with correct password hashes."
echo "JWT configuration has been set up."
echo ""
