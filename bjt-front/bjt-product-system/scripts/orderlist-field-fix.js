...
/**
 * OrderList页面字段标准化修复脚本
 * 修复order.json中的包装尺寸字段显示，符合CSV标准要求
 */
...
const CONFIG = {
  ...
  fixes: {
    ...
    'frontend/src/i18n/locales/en/order.json': [
      {
        path: 'products.properties.package_size_cm',
        old: 'Package Size (cm)',
        new: 'Packaging Dim. (cm)',
        description: '包装尺寸字段标准化：Package Size → Packaging Dim.'
      }
    ]
  }
};
...
function main() {
  ...
  if (dryRun && totalChanges > 0) {
    ...
    log('   node scripts/orderlist-field-fix.js', 'yellow');
  }
