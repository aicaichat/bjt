...
// 获取当前编辑的主机型号ID
$host_model_id = isset($_GET['id']) ? intval($_GET['id']) : 0;
...
<div class="bjt-host-model-form">
  <div class="bjt-page-header">
    <div class="bjt-page-title-container">
      <h1 class="bjt-page-title">编辑主机型号</h1>
  ...
  <form id="host-model-form" class="bjt-form bjt-card" data-id="<?php echo esc_attr($host_model_id); ?>">
    <div class="bjt-card-body">
      <div class="bjt-loading-indicator">正在加载数据...</div>
      
      <div class="bjt-form-tabs" style="display: none;">
...
jQuery(document).ready(function($) {
  // 获取主机型号ID
  const hostModelId = $('#host-model-form').data('id');
  
  // 加载主机型号数据
  loadHostModelData(hostModelId);
  ...
  $('#host-model-form').on('submit', function(e) {
    e.preventDefault();
    updateHostModel(hostModelId);
  });
  
  // 加载主机型号数据
  function loadHostModelData(id) {
    $.ajax({
      ...
      success: function(data) {
        // 填充表单数据
        fillHostModelForm(data);
        
        // 隐藏加载指示器，显示表单
        $('.bjt-loading-indicator').hide();
        $('.bjt-form-tabs').show();
      },
      error: function(error) {
        console.error('加载主机型号数据失败', error);
        $('.bjt-loading-indicator').text('加载数据失败，请刷新页面重试');
      }
    });
  }
  
  // 填充表单数据
  function fillHostModelForm(data) {
    // 基本信息
    $('#product_line').val(data.product_line);
    $('#model').val(data.model);
    $('#title_cn').val(data.title_cn);
    ...
    // 图片
    $('#image1_url').val(data.image1_url);
    if (data.image1_url) {
      $('#image1-preview').html(`<img src="${data.image1_url}" alt="主图预览">`);
    }
    
    $('#image2_url').val(data.image2_url);
    if (data.image2_url) {
      $('#image2-preview').html(`<img src="${data.image2_url}" alt="副图预览">`);
    }
    
    // PDF文件
    $('#explosion_diagram_pdf').val(data.explosion_diagram_pdf);
    if (data.explosion_diagram_pdf) {
      // 提取文件名
      const fileName = data.explosion_diagram_pdf.split('/').pop();
      $('#pdf-file-name').text(fileName);
    }
  }
  
  // 更新主机型号
  function updateHostModel(id) {
    // 收集表单数据
    const formData = {
    ...
    $.ajax({
      ...
      success: function(response) {
        showNotification('success', '主机型号更新成功');
      ...
      error: function(error) {
        console.error('更新主机型号失败', error);
        showNotification('error', '更新失败: ' + (error.responseJSON?.message || '未知错误'));
      },
      complete: function() {
