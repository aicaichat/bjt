...
fix_api_parameters() {
    ...
    for file in "${files[@]}"; do
        ...
        if grep -q "machineparts" "$file" && ! grep -q "machineparts.*status=publish" "$file"; then
            ...
            sed -i.tmp -E 's|(/machineparts\?[^"]*)|\\1\&status=publish|g; s|(/machineparts)"|\1?status=publish"|g' "$file"
        ...
        for api in "accessories" "consumables"; do
            if grep -q "$api" "$file" && ! grep -q "$api.*status=publish" "$file"; then
                ...
                sed -i.tmp -E "s|(/$api\\?[^\"]*)|\\1\\&status=publish|g; s|(/$api)\"|\\1?status=publish\"|g" "$file"
                sed -i.tmp -E 's|status=publish&status=publish|status=publish|g' "$file"
                rm -f "$file.tmp"
...
verify_fixes() {
    ...
    if [ -f "./simple-status-check.sh" ]; then
...
main() {
    ...
    if [ ! -d "../../frontend/src" ]; then
        log_error "请在项目根目录或tools/product-status-tools目录运行此脚本"
        exit 1
    fi
    ...
    echo -e "${BLUE}📖 详细指南: ./product-status-checklist.md${NC}"
}
