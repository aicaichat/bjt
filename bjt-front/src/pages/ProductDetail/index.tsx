      ...
      <div className="product-detail-content">
        ...
        <div className="product-detail-info">
          ...
          <div className="product-price-container">
            <div className="product-price">
              ...
              {product.price.discount && (
                <span className="discount-badge">省{product.price.discount}%</span>
              )}
            </div>
            
            {product.price.tiers.length > 1 && (
              <div className="tiered-prices">
                <h4>批量价格:</h4>
                <ul>
                  {product.price.tiers.map((tier, index) => (
                    <li key={index}>
                      {tier.range}: ¥{tier.price.toLocaleString()}/件
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div className="product-inventory">
            <span className={`inventory-status ${product.stock.status}`}>
              {product.stock.text}
            </span>
            {product.stock.status !== 'out_of_stock' && (
              <span className="inventory-quantity">
                库存: {product.stock.quantity} 件
              </span>
            )}
          </div>
