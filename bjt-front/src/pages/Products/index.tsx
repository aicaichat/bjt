...
const Products: React.FC = () => {
  ...
  const [selectedCapacity, setSelectedCapacity] = useState('全部产能');
  const [selectedPrice, setSelectedPrice] = useState('全部价格');
  const [selectedSort, setSelectedSort] = useState('默认排序');
  const [userRole, setUserRole] = useState('customer'); // 可选值: 'admin', 'partner', 'customer'
  ...
  const [activeTab, setActiveTab] = useState<string>('Void Fill');
  ...
  const addToCart = async (productId: string, basePrice: number) => {
    try {
      ...
      // 模拟产品规格信息
      const specs: CartItemSpec = {
        model: product.model || 'N/A',
        partNumber: `BJT-${productId}-${new Date().getFullYear()}`,
        productName: product.name,
        voltage: selectedVoltage,
        frequency: '50Hz',
        palletSize: product.palletSize || '120 × 80 × 160 cm',
        palletQty: product.palletQty || '24件'
      };
      
      // 确定产品类型
      const type = product.category === 'machine' ? 'machine' : 'accessory';
      
      await apiService.cart.addToCart(productId, quantity, selectedVoltage, specs, type);
              ...
              <div key={product.id} className="product-item">
                ...
                <div className="item-pricing">
                  <div className="price-wrapper">
                    <div className="current-price">¥{getPrice(product).toLocaleString()}</div>
                    {userRole === 'partner' && (
